...
public class MusicToClientPacket implements Packet {
    ...
    public MusicToClientPacket(PacketByteBuf packetByteBuf) {
        this(
    ...
    public MusicToClientPacket(int x, int y, int z, String url, int timeSecond, String songName) {
        this(x, y, z, url, timeSecond, songName, 0, 0);
    }

    public MusicToClientPacket(int x, int y, int z, String url, int timeSecond, String songName, int startTick) {
        this(x, y, z, url, timeSecond, songName, startTick, 0);
    }

    public MusicToClientPacket(int x, int y, int z, String url, int timeSecond, String songName, int startTick, int playSessionId) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.url = url;
    ...
    public static void applyClientPlayback(EntityPlayer entityPlayer, int x, int y, int z,
        ...
        if (StringUtils.isBlank(safeUrl)) {
            ClientMusicPlayer.stopAt(x, y, z, "sync_blank_url");
            return;
        }
        ...
        if (safePlaySessionId > 0) {
            ...
            if (currentSession > 0 && currentSession != safePlaySessionId) {
                ClientMusicPlayer.stopAt(x, y, z, "sync_session_changed");
            }
        ...
        if (ClientMusicPlayer.isPlayingAt(x, y, z)) {
            ClientMusicPlayer.stopAt(x, y, z, "sync_restart_playing_at_pos");
            if (GeneralConfig.ENABLE_DEBUG_MODE) {
        ...
        MusicPlayManager.play(playbackResolveKey(x, y, z), safeUrl, safeSongName, resolved -> {
            if (!isMusicPlayerPresent(x, y, z)) {
                ClientMusicPlayer.stopAt(x, y, z, "resolve_stale");
                return null;
            }
            if (!ClientMusicPlayer.isPendingAtSource(x, y, z, sourceId)) {
                return null;
    ...
    private static String playbackResolveKey(int x, int y, int z) {
        return x + "," + y + "," + z;
    }
    ...
    private static boolean isMusicPlayerPresent(int x, int y, int z) {
        Minecraft minecraft = Minecraft.getMinecraft();
        if (minecraft == null || minecraft.theWorld == null) {
            return false;
        }
        return minecraft.theWorld.getBlockTileEntity(x, y, z) instanceof TileEntityMusicPlayer;
    }
